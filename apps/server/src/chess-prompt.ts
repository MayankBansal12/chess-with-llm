...
const getColorName = (color: ChessColor): "Black" | "White" =>
  color === "w" ? "White" : "Black";

const getOpponentColorName = (color: ChessColor): "Black" | "White" =>
  color === "w" ? "Black" : "White";

export const buildTournamentSystemPrompt = (color: ChessColor): string => {
  const colorName = getColorName(color);
  const opponentName = getOpponentColorName(color);
  const ownedPieceCase = color === "w" ? "uppercase" : "lowercase";
  const opponentPieceCase = color === "w" ? "lowercase" : "uppercase";

  return `You are playing ${colorName} in a real chess game against another language model. Maximize your chance of winning against the strongest plausible ${opponentName} replies. In the ASCII board, ${ownedPieceCase} pieces are yours (${colorName}) and ${opponentPieceCase} pieces belong to ${opponentName}. Analyze the supplied position privately: first identify checks and immediate threats; then examine forcing moves (mates, checks, captures, and threats); compare serious candidates by calculating ${opponentName}'s strongest response; reject moves that allow mate, hang material, leave pieces undefended, or miss a tactical threat; and use king safety, activity, pawn structure, and endgame prospects to break ties. Choose only from the supplied legal moves. Reply with strict JSON in the form {"move":"e2e4","message":"I claim central space and open lines for my pieces."}. The move must use UCI notation. The message must be one sentence of at most 120 characters and explain the move's concrete purpose in the current position. Analyze privately and expose only the JSON, without markdown.`;
};
...
export const buildTournamentModelPrompt = (
  position: ModelPosition,
  color: ChessColor,
  invalidMove?: string | null
): string => {
  ...
  return `You are ${colorName} and it is your move. Treat the FEN as the authoritative position and assume ${opponentName} will find the strongest reply.\n\nCurrent FEN:\n${position.fen}\n\nCurrent game PGN:\n${position.pgn || STARTING_POSITION_PGN}\n\nPosition facts:\n- Last move: ${position.lastMove}\n- ${colorName} in check: ${position.isCheck ? "yes" : "no"}\n- Castling rights: ${position.castlingRights}\n- Material balance: ${position.materialBalance}\n- Draw status: ${position.drawStatus}\n\nPiece symbols: P/p = pawn, N/n = knight, B/b = bishop, R/r = rook, Q/q = queen, and K/k = king. ${ownedPieceCase} pieces are yours (${colorName}); ${opponentPieceCase} pieces belong to ${opponentName}.\n\nASCII board representation for current position:\n${position.asciiBoard}\n\nLegal moves, formatted as UCI (SAN):\n${legalMoves}${retryInstruction}\n\nPrivately compare the strongest candidates and check the chosen move for ${opponentName}'s best tactical reply. Return exactly one legal UCI move and a concrete explanation in the required JSON format.`;
};
